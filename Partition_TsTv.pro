Function Partition_TsTv, LST, Fveg, Ts_min, Ts_max, Tv_min, Tv_max
...
;;****Det. TVEG (Tvg) en T-Fveg
  ...
  ;emis_s=0.957
  ;emis_s=0.99
  ;emis=(1-Fveg)*emis_s + Fveg*emis_v         ;Fveg(NDVI)
    ;Ts = ((LST^4.*emis - Tvg^4.*Fveg*emis_v)/((1 - Fveg)*emis_s))^0.25
  Ts = min([Ts,Ts_max])
  Ts = max([Ts,Ts_min])
