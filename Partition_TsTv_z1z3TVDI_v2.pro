Function Partition_TsTv_z1z3TVDI_v2, LST, Fveg, Ts_min, Ts_max, Tv_min, Tv_max
  ;;NIVEL IN SITU
...
;    Ts = Ts_min + (Ts_max - Ts_min)*(LST - Tj)/(Th - Tj) ; = (LST - Tvg*Fveg)/(1 - Fveg)f
    coef_min = LINFIT( [ Fveg , 1 ]  ,  [ LST , Tv_min] )
    coef_max = LINFIT( [ Fveg , 1 ]  ,  [ LST , Tv_max ] )
    Tsol_min = coef_min[0]                                  ;siempre caeran dentro de [Ts_min,Ts_max] pq estan dentro de diagonales
    Tsol_max = coef_max[0]
    Ts = (Tsol_min + Tsol_max)/2.
  endif
  ...
  if (LST lt Diag_BD  and  LST lt Diag_AC) then begin
    ...
    coef_max_ZC = LINFIT( [ 0 , Fveg ]  ,  [ Ts_min , LST ] )
  ...
  if (LST le Diag_BD  and  LST ge Diag_AC) then begin
    coef_min = LINFIT( [ 0 , Fveg ]  ,  [ Ts_max , LST ] )
    coef_max = LINFIT( [ 0 , Fveg ]  ,  [ Ts_min , LST ] )
    Tvg_min = coef_min[0] + coef_min[1] * 1               ;siempre caeran dentro de [Tv_min,Tv_max] pq estan dentro de diagonales
    Tvg_max = coef_max[0] + coef_max[1] * 1
;    Tvg_min = min([Tvg_min,Tv_max])
;    Tvg_min = max([Tvg_min,Tv_min])
;    Tvg_max = min([Tvg_max,Tv_max])
;    Tvg_max = max([Tvg_max,Tv_min])
