Function EB_veg_mo, epsilon, alpha, cg, zr, h, rss, ta, ua, rg, rha, SIGMA
;;rss = canopy resistance
...
;  ===================================================================================*/
  ...
  NITER_rah = 100
  NITER_H = 100
  NITER_T = 100; // number of iterations considered for running the loop on Ts minimization
  EPST = 0.01; // Temperature accuracy
  EPSF = 0.1;  // Sensible heat H accuracy
  
  ;     /* local variables*/
  d=2./3*h ;  // zero plane displacement (0 for bare soils), VEGETATION: d=2/3*h      // z0m=0.123*h    //  z0h=0.1*z0m
  z0m=0.123*h
...
;;;// Loop on Ts
  fts=999
  while ( abs(fts) gt EPST ) do begin
    h_mo = 1;
    le_mo = 1;
    ts = ts1;
...
;;;// Loop on U
      ...
      if (nra eq 200) then print,("Reached maximum number of iterations for computing the wind speed");
      
      ;// Resistance
      ;rah = (alg - psih)/(KARMAN * ustar)
      rah = (alg_h - psih)/(KARMAN * ustar); // Bastiaanssen 2000
;      rah = (alg_h - psih + psih0)/(KARMAN * ustar); psih0 = MO.calc_Psi_H(z_0H/L)
      
      Fluxes_ts = energy_fluxes_rss(ts, epsilon, alpha, cg, zr, z0m, rss, ta, ua, rg, rha, rah, SIGMA, RHOCP, GAMMA);(ET,G,H,Rn) 
      ;print, Fluxes_ts     
    ...
    dfts = derivative_fts_rss(ts, epsilon, alpha, cg, zr, z0m, rss, ta, ua, rg, rha, rah, SIGMA, RHOCP, GAMMA, DELTAT);
  ...
  FLUXES_ts = energy_fluxes_rss( ts1, epsilon, alpha, cg, zr, z0m, rss, ta, ua, rg, rha, rah, SIGMA, RHOCP, GAMMA);
  ...
  Function energy_fluxes_rss, ts, epsilon, alpha, cg, zr, z0m, rss, ta, ua, rg, rha, rah, SIGMA, RHOCP, GAMMA
  ...
  Function derivative_fts_rss, ts, epsilon, alpha, cg, zr, z0m, rss, ta, ua, rg, rha, rah, SIGMA, RHOCP, GAMMA, DELTAT
    ;/*=============================================================================
    ;Derivative of the cost function df(Ts) / dTs for Rss from Sellers et al., 1992
    ;===============================================================================*/
    Flux_ts = energy_fluxes_rss( ts, epsilon, alpha, cg, zr, z0m, rss, ta, ua, rg, rha, rah, SIGMA, RHOCP, GAMMA);
    
    fts = cost_function(Flux_ts[0], Flux_ts[1], Flux_ts[2], Flux_ts[3]);
    
    Flux_DELTAT = energy_fluxes_rss( ts + DELTAT, epsilon, alpha, cg, zr, z0m, rss, ta, ua, rg, rha, rah, SIGMA, RHOCP, GAMMA);
  ...
  Function longwave, ea, ta, SIGMA
    ...
    ;==========================================================*/
    epsa = 0.553*(ea/100)^0.143; --> ea:[Pa]; 1.24*(ea/(100*ta))^0.143 ;-->ea:[Pa] ta:[K]; 
    ldown = epsa*SIGMA*(ta^4); = 1.24*(ea/ta)^0.143 * ta^4. * SB
    return, ldown
  end
  
;  Function G_Rn, alb, Ts, NDVI
