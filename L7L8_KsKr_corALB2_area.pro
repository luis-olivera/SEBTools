pro L7L8_KsKr_corALB2_area
  ...
  ;EBmet = ['EBsolveg','EBsolveg\TvminTa']
  ...
  ;FILE_MKDIR, 'L7\Fvg\Fvg1', 'L7\Fvg\Fvg2', 'L8\Fvg\Fvg1', 'L8\Fvg\Fvg2'
...
;      path_Tends_2 = pat + 'Insitu\'+ manip + '\Results\' + met[mm] +'\TvTs_fc\TVDI_fc\LSTcor_alb0.300_emis0.957_z0m0.0010_albvg0.30\TsTv_Tends_EB_mo_1100.txt'
      ...
      outdir = 'Results\Fvg'+ string(strcompress(pp,/remove_all)) + '\TVDI_fc\' + met[mm] + '\Cor_alb2\';v2\
      FILE_MKDIR, 'Results\Fvg'+ string(strcompress(pp,/remove_all)) + '\TVDI_fc\' + met[mm] + '\EF'
      FILE_MKDIR, outdir+'LST', outdir+'EF'
      ...
      for year=2016,2016 do begin
        ...
        openw, 1, outdir + 'Tends_LST-fc_' + YR + '.txt'
...
;          ;Fvg_ENDMBmax = mean(Fvg,/nan) ;-->default
          ...
          if n_elements(pix) gt Min_data*ss then begin
            ...
            if met[mm] eq 'Regresion' then $
              LSTend = Tends_Fvg_IMA_regresion(LST, fc, date=date, outplot=outplot, /plot_, bin_x=0.01);, 
            ...
            if met[mm] eq 'Regresion' then $
              LSTend_cor = Tends_Fvg_IMA_regresion(LSTcor, fc, date=date, outplot=outplot);, /plot
            if met[mm] eq 'Regresion_dev' then $
              LSTend_cor = Tends_Fvg_IMA_regresion(LSTcor, fc, /dev, date=date, outplot=outplot);, /plot--> +-stddev, guardar plot con date en el nom
            if met[mm] eq 'Regresion_dev0.5' then $
              LSTend_cor = Tends_Fvg_IMA_regresion(LSTcor, fc, /dev, f_dev=0.5, date=date, outplot=outplot);, /plot--> +-0.5stddev
            if met[mm] eq 'EBsolveg\TvminTa' OR met[mm] eq 'EBsolveg' then begin
              LSTend_cor = LSTend_EB          ; Ts -->alb_s=0.2 ; Tv -->alb_vg=0.2
            endif
  ...
  ;            Ts_Tvg_Z = Partition_TsTv_z1z3TVDI_IMA_z2Ks1(LST, Fvg, Ts_min, Ts_max, Tv_min, Tv_max)
            ...
            Ts_Tvg_Z = Partition_TsTv_TVDI_fc_IMA(LSTcor, fc, Ts_min, Ts_max, Tv_min, Tv_max)
            Ts = Ts_Tvg_Z[*,*,0]
            Tv = Ts_Tvg_Z[*,*,1]
            ;Zona = Ts_Tvg_Z[*,*,2]
            ...
            p.Save, outdir + 'Poligone\LST_fc_' + date + '_'+Hra_obj+'.png', BORDER=10, RESOLUTION=100
            p.close

          endelse
           
;          bias = mean((LSTcor - LST),/nan)
;          print, bias
;          p=plot(LST,LSTcor,'*')
;          p=plot(alb, LSTcor-LST,'*')
;          p.close
          printf, 1, date, sensor, LSTend_cor, format="(2a8, 4f7.2)"
          ...
          write_tiff, outdir + 'LST\LST_' + date + '_'+Hra_obj+'.tif', LSTcor, /float, geotiff=g_tags
...
function plot_Tend_EB, fvg, LST, LSTcor, LSTend, LSTend_cor, LSTend_EB
  ...
  P= plot(fvg(where(finite(fvg))), LST(where(finite(fvg))), '*', xrange=[0,1], $
    xtitle="Fractional total vegetation cover [-]", ytitle="Surface Temperature [K]", $
    TITLE='Landsat LST', /CURRENT, LAYOUT=[2,1,1])
  p2= plot([0, 1], [LSTend_EB[1], LSTend_EB[3]],'r2',/overplot, NAME='Energy Balance'); color=2
  p3= plot([0, 1], [LSTend_EB[0], LSTend_EB[2]],'b2',/overplot); color=4
  p4= plot([0, 1], [LSTend[1], LSTend[3]],'r2--',/overplot, NAME='Imaged-based'); color=2
  ...
  P_= plot(fvg(where(finite(fvg))), LSTcor(where(finite(fvg))), '*', xrange=[0,1], $
    xtitle="Fractional total vegetation cover [-]", ytitle="Surface Temperature [K]", $
    TITLE='Normalized Landsat LST', /CURRENT, LAYOUT=[2,1,2])
  p2= plot([0, 1], [LSTend_EB[1], LSTend_EB[3]],'r2',/overplot, NAME='Energy Balance'); color=2
  p3= plot([0, 1], [LSTend_EB[0], LSTend_EB[2]],'b2',/overplot); color=4
  p4= plot([0, 1], [LSTend_cor[1], LSTend_cor[3]],'r2--',/overplot, NAME='Imaged-based'); color=2
