pro Tends_EBsoilveg_mo_inst_Partition
  close, /all
  ;  EB_soil_mo
  ;  Partition_TsTv
  ...
  ;  color_8
  manip='R3_Gravitaire_2015-2016'
  root = 'D:\CESBIO\Region\Maroc\Area\R3\Insitu\'
  path = root + manip + '\Results' + '\'  
  pat_meteo_day = root + manip + '\climat_R3_day_2016.csv'
  ...
  zr = 2.25;3           ;// height of data acquisition [m]
...
;;DAILY
  ...
  NDVI=meteo_day.FIELD18
  
  
  for ff=0,0 do begin
    for rr=0,0*(n_elements(rss_met)-1) do begin;rss_met
    
      for tt=0,0 do begin; n_elements(Tdat)-1; Tdat
      
        ;;;;Instantanean
        if Tdat[tt] eq 'LSTcor_' then $
          pat_meteo = root + manip + '\climat-R3_30min_LSTcor.csv' $ ;LST de Tb apogee
        else $
          pat_meteo = root + manip + '\climat-R3_30min.csv'         ;Tb medido del apogee (dato bruto)
        ...
        for pp=0,0 do begin;n_elements(partition)-1; partition
          for aa=0,n_elements(alpha_min)-1 do begin;alpha_min
            for zz=0,0 do begin;z0m
                ...
                For j=0, na-1 do begin
                  ...
                  if nj eq 1 then begin
                    ...
                    if EB eq 'EBsolveg\' OR EB eq 'EBsolveg\TvminTa\' then begin
                      Tvmax_eb = EB_veg_mo(epsilon, alpha_veg, cg_vg, zr,h, 1500, ta[i], ua[i], rg[i], rha[i], SIGMA)
                      Tvmax_eb = Tvmax_eb[0]
                      if EB eq 'EBsolveg\TvminTa\' then begin
                        Tvmin_eb = ta[i]
                      endif else begin
                        Tvmin_eb = EB_veg_mo( 0.985, alpha_veg, cg_vg, zr, h, rss_min, ta[i], ua[i], rg[i], rha[i], SIGMA)
                        Tvmin_eb = Tvmin_eb[0]
                      endelse
                    endif else begin
              ...
              path_meteo= root + 'Meteo_2016_ble\meteobloc123_mean' + string(hr_i, FORMAT='(I02)') + string(hr_f, FORMAT='(I02)') + '.txt'
              openw, 1, path_out
              openw, 2, path_meteo
              for j=0, na-1 do begin
